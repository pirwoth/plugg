                ...
                <div className="space-y-5">
                  {isArtistsLoading ? (
                    ...
                    topArtists?.slice(0, 5).map((artist) => (
                      ...
                      <button onClick={() => router.push(`/profile/${artist.id}`)} className="bg-primary text-primary-foreground px-5 py-2 rounded-full text-xs font-black hover:scale-105 active:scale-95 transition-all shadow-md">View</button>
                    </div>
                    ))
                  )}
                </div>
