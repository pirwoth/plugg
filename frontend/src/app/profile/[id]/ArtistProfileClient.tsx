...
export default function ArtistProfileClient({ id }: { id: string }) {
  ...
  const { data: artist, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [songsRes, playlistsRes] = await Promise.all([
        ...
        supabase
          .from('playlists')
          .select('*, playlist_stats(*)')
          .eq('artist_id', artistIdNum)
          .order('first_seen_at', { ascending: false })
      ]);

      if (songsRes.error) console.error("Songs fetch error:", songsRes.error);
      if (playlistsRes.error) console.error("Playlists fetch error:", playlistsRes.error);

      const allTracks = [
        ...(songsRes.data || []).map(row => ({ ...row, type: 'song' })),
        ...(playlistsRes.data || []).map(row => ({ ...row, type: 'playlist' }))
      ].sort((a, b) => new Date(b.first_seen_at || 0).getTime() - new Date(a.first_seen_at || 0).getTime());

      const mappedSongs: Song[] = allTracks.map(row => {
        const statsKey = row.type === 'song' ? 'song_stats' : 'playlist_stats';
        const statsArray = Array.isArray(row[statsKey]) ? row[statsKey] : [];
        const latestStats = statsArray.sort((a: any, b: any) => new Date(b.date || 0).getTime() - new Date(a.date || 0).getTime())[0] || { plays: 0, downloads: 0 };
        return {
